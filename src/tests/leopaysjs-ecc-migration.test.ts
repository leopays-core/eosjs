...
describe('ecc Migration', () => {
  ...
  const legacyPublicKeys = [
    'LPC7tgwU6E7pAUQJgqEJt66Yi8cWvanTUW8ZfBjeXeJBQvhTU9ypi',
    'LPC8VaY5CiTexYqgQZyPTJkc3qvWuZUi12QrZL9ssjqW2es6aQk2F',
    'LPC7VGhqctkKprW1VUj19DZZiiZLX3YcJqUJCuEcahJmUCw3wJEMu',
  ];
  ...
  it('verifies `privateToPublic` function is consistent between ecc objects', () => {
    console.warn = jest.fn();
    const eccPublicKey = ecc.privateToPublic(privateKeys[0], 'LPC');
    const eccMigrationPublicKey = eccMigration.privateToPublic(privateKeys[0], 'LPC');
  ...
  it('verifies `isValidPublic` function is consistent between ecc objects', () => {
    console.warn = jest.fn();
    const eccValid = ecc.isValidPublic(legacyPublicKeys[0], 'LPC');
    const eccMigrationValid = eccMigration.isValidPublic(legacyPublicKeys[0], 'LPC');
    expect(console.warn).toHaveBeenCalledWith('Argument `pubkey_prefix` is deprecated, ' +
      'keys prefixed with PUB_K1_/PUB_R1_/PUB_WA_ going forward');
  ...
  it('verifies `isValidPublic` function is consistent during an error', () => {
    console.warn = jest.fn();
    const eccValid = ecc.isValidPublic('publickey', 'LPC');
    const eccMigrationValid = eccMigration.isValidPublic('publickey', 'LPC');
    expect(console.warn).toHaveBeenCalledWith('Argument `pubkey_prefix` is deprecated, ' +
      'keys prefixed with PUB_K1_/PUB_R1_/PUB_WA_ going forward');
    expect(eccValid).toEqual(eccMigrationValid);
